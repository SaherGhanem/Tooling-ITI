...
def extractFemaleNames(filename,letter):
  ...
  data = open_file.read()
  list_ = re.findall(r"<td>(\d+)</td><td>(\w+)</td><td>(\w+)</td>",data)
  for i in list_:
    if letter == i[2][0]:
...
def extractDocAndPopularityDate(filename):
  # <h1>Popular Names by Birth Year</h1>(\w+ \d+, \d+)</td>
  # >(\w+ in \d+)</h3>
  list1= []
  open_file = open(filename,'r')
  data = open_file.read()
  st1 = re.findall(r"<h1>Popular Names by Birth Year</h1>(\w+ \d+, \d+)</td>",data)
  st2 = re.findall(r">(\w+ in \d+)</h3>",data)
  #list1.append(str(st1)+' '+"and"+' '+str(st2))
  
  #print(str(st1)+' '+"and"+' '+str(st2))
  return str(st1[0])+' '+"and"+' '+str(st2[0])
